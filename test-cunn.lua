...
function cunntest.copies()
   ...
   t:normal()
   t2 = t:clone()
   tester:asserteq( t:add(-1,t2):abs():max(), 0, 'simple copy')

   -- transpose copy
   t:normal()
   t3 = t:transpose(1,2)
   t4 = t3:clone()
   tester:asserteq( t3:add(-1,t4):abs():max(), 0, 'transpose copy')
   
   -- unfold copy
   t:normal()
   t5 = t:unfold(2,5,1)
   t6 = t5:clone()
   tester:asserteq( t5:add(-1,t6):abs():max(), 0, 'transpose copy')
   ...
   tc = tc:transpose(1,2)
   local t2 = tc:float()
   tester:asserteq(t:transpose(1,2):add(-1,t2):abs():max(), 0, 'host copy, plus transpoe')
end
...
function cunntest.Tanh_forward()
   ...
   tester:assertlt(error:abs():max(), precision_forward, 'error on state (forward) ')
end
...
function cunntest.Tanh_backward()
   ...
   tester:assertlt(error:abs():max(), precision_backward, 'error on state (backward) ')
end
...
function cunntest.Max_forward()
   ...
   tester:assertlt(error:abs():max(), 1e-8, 'error on indices ')
end
...
function cunntest.SpatialConvolution_backward()
   ...
   tester:assertlt(error:abs():max(), precision_backward, 'error on state (backward) ')
   tester:assertlt(werror:abs():max(), precision_backward, 'error on weight (backward) ')
   tester:assertlt(berror:abs():max(), precision_backward, 'error on bias (backward) ')
end
...
function cunntest.SpatialMaxPooling_forward()
   ...
   s = { {},1,1,{1,10} }
   tester:asserteq(error_ind:max(), 0, 'error on indices (forward) ')
end
...
function cunntest.mse()
   ...
   tester:assertlt(math.abs(fout-cout), precision_forward, 'error  on output')
   local gerr = cgin:float() - fgin
   tester:assertlt(gerr:abs():max(), precision_forward, 'error  on gradInput')

   tester:assertlt(math.abs(fout-cout2), precision_forward, 'error  on output - 2')
   local gerr2 = cgin2:float() - fgin
   tester:assertlt(gerr2:abs():max(), precision_forward, 'error  on gradInput -2')
end
