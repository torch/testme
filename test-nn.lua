...
function nntest.Add()
   ...
   tester:assertlt(err,precision, 'error on bias ')

   local err = jac.testJacobianUpdateParameters(module, input, module.bias)
   tester:assertlt(err,precision, 'error on bias [direct update]')

   for t,err in pairs(jac.testAllUpdate(module, input, 'bias', 'gradBias')) do
      tester:assertlt(err, precision, string.format(
                         'error on bias [%s]', t))
   end
...
function nntest.Power()
   ...
   tester:asserteq(err, 0, torch.typename(module) .. ' - forward err ')

   local ins = math.random(5,10)
   local pw = torch.random(2,7)
   local input = torch.Tensor(ins):zero()
...
function nntest.WeightedMSECriterion()
   ...
   local ferr, berr = jac.testIO(module, input)
   tester:asserteq(0, ferr, torch.typename(module) .. ' - i/o forward err ')
   tester:asserteq(0, berr, torch.typename(module) .. ' - i/o backward err ')
end
--]]
...
function nntest.LogSoftmax()
   ...
   local err = jac.testJacobian(module,input)
   tester:assertlt(err,expprecision, 'error on state ')
...
--    print(module.gradWeight:size())
   ...
   local err = jac.testJacobian(module, input)
   tester:assertlt(err, precision, 'batch error on state ')
   
   local err = jac.testJacobianParameters(module, input, module.weight, module.gradWeight)
   tester:assertlt(err , precision, 'batch error on weight ')
   
   local err = jac.testJacobianParameters(module, input, module.bias, module.gradBias)
   tester:assertlt(err , precision, 'batch error on bias ')

   local err = jac.testJacobianUpdateParameters(module, input, module.weight)
   tester:assertlt(err , precision, 'batch error on weight [direct update] ')
   
   local err = jac.testJacobianUpdateParameters(module, input, module.bias)
   tester:assertlt(err , precision, 'batch error on bias [direct update] ')
...
function nntest.SpatialFullConvolutionCompare()
    ...
    tester:assertlt(o1:dist(o2), precision, 'error on output')
    ...
    tester:assertlt(gi1:dist(gi2), precision, 'error on gradInput')
    ...
    for k=1,tt:size(1) do
      tester:assertlt(module1.gradWeight[k]:dist(module2.gradWeight[tt[k][1]][tt[k][2]]),precision,'error on gradWeight ' .. k)
    end
    tester:assertlt(module1.gradBias:dist(module2.gradBias),precision,'error on gradBias ')
end
...
local function batchcompare(smod, sin, plist)
   ...
   tester:assertTensorEq(sout,bout:select(1,1), 1e-8, 'batchcompare error on output')
   tester:assertTensorEq(sgin,bgin:select(1,1), 1e-8, 'batchcompare error on gradInput')

   for i,v in pairs(plist) do
      tester:assertTensorEq(smod[v],bmod[v], 1e-8, 'batchcompare error on ' .. v)
   end
end
...
function nntest.SpatialMaxPooling()
   ...
   local err = jac.testJacobian(module, input)
   tester:assertlt(err, precision, 'error on state (Batch) ')

   local ferr, berr = jac.testIO(module, input)
   tester:asserteq(ferr, 0, torch.typename(module) .. ' - i/o forward err (Batch) ')
   tester:asserteq(berr, 0, torch.typename(module) .. ' - i/o backward err (Batch) ')
...
function nntest.Module_getParameters_1()
   ...
   tester:asserteq((p[{ {1,100} }] - n.modules[1].weight):norm(), 0, 'getParameters(): weights wrong')
   tester:asserteq((p[{ {101,110} }] - n.modules[1].bias):norm(), 0, 'getParameters(): bias wrong')
end
...
function nntest.Module_getParameters_2()
   ...
   tester:asserteq((p[{ {111,210} }] - n.modules[2].weight):norm(), 0, 'error when appending new module')
   tester:asserteq((p[{ {211,220} }] - n.modules[2].bias):norm(), 0, 'error when appending new module')
end
...
function nntest.Module_getParameters_3()
   ...
   tester:asserteq((p[{ {1,100} }] - n.modules[1].weight):norm(), 0, 'error when using cloning')
   tester:asserteq((p[{ {101,110} }] - n.modules[1].bias):norm(), 0, 'error when using cloning')

   tester:asserteq((p[{ {111,210} }] - n.modules[2].weight):norm(), 0, 'error when using cloning')
   tester:asserteq((p[{ {211,220} }] - n.modules[2].bias):norm(), 0, 'error when using cloning')

   tester:asserteq((p[{ {111,210} }] - n.modules[1].weight):norm(), 0, 'error when using cloning')
   tester:asserteq((p[{ {211,220} }] - n.modules[1].bias):norm(), 0, 'error when using cloning')

   n:reset()

   tester:assertgt((p[{ {111,210} }] - n.modules[1].weight):norm(), 0, 'error when using cloning')
   tester:assertgt((p[{ {211,220} }] - n.modules[1].bias):norm(), 0, 'error when using cloning')
end
...
function nntest.Module_getParameters_4()
   ...
   tester:asserteq((p[{ {221,320} }] - n.modules[3].weight):norm(), 0, 'error when using cloning')
   tester:asserteq((p[{ {321,330} }] - n.modules[3].bias):norm(), 0, 'error when using cloning')

   tester:asserteq(p:nElement(), 3*(10*10+10), 'error: incorrect number of elements in flat vector')
end
...
function nntest.Module_getParameters_5()
   ...
   tester:asserteq((p[{ {1,100} }] - n.modules[1].weight):norm(), 0, 'error when using cloning+sharing')
   tester:asserteq((p[{ {101,110} }] - n.modules[1].bias):norm(), 0, 'error when using cloning+sharing')

   tester:asserteq((p[{ {1,100} }] - n.modules[2].weight):norm(), 0, 'error when using cloning+sharing')
   tester:asserteq((p[{ {101,110} }] - n.modules[2].bias):norm(), 0, 'error when using cloning+sharing')
   ...
   tester:asserteq(p:nElement(), (10*10+10), 'error: incorrect number of elements in flat vector')
end
...
function nntest.Module_getParameters_6()
   ...
   tester:asserteq((p[{ {111,210} }] - n.modules[3].weight):norm(), 0, 'error when using cloning+sharing')
   tester:asserteq((p[{ {211,220} }] - n.modules[3].bias):norm(), 0, 'error when using cloning+sharing')

   tester:asserteq(p:nElement(), 2*(10*10+10), 'error: incorrect number of elements in flat vector')
end
...
function nntest.Module_getParameters_7()
   ...
   tester:asserteq((p[{ {1,100} }] - n1.modules[1].weight):norm(), 0, 'error when using cloning+partial realloc')
   tester:asserteq((p[{ {101,110} }] - n1.modules[1].bias):norm(), 0, 'error when using cloning+partial realloc')

   tester:asserteq((p[{ {111,120} }] - nf.modules[2].weight):norm(), 0, 'error when using cloning+partial realloc')
   tester:asserteq((p[{ {121,121} }] - nf.modules[2].bias):norm(), 0, 'error when using cloning+partial realloc')

   tester:asserteq(p:nElement(), 121, 'error: incorrect number of elements in flat vector')
end

math.randomseed(os.time())

print('')
print('Testing nn with type = double')
print('')
...
--[[
-- TODO: how do we do proper testing with single precision?
-- using max() instead of norm()?
print('')
print('Testing nn with type = float')
